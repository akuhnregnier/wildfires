...
if __name__ == '__main__':
    ...
    exog_names = [exog_name_map.get(s[0], s[0]) for s in
                  selected_datasets if s[0] != burned_area_name]
    ...
    exog_data['temperature range'] = (exog_data['maximum temperature']
                                      - exog_data['minimum temperature'])
    ...
    data_map_plot(np.mean(global_mask, axis=0),
                  name='Mean available data mask',
                  filename='mean_avail_mask.png')
    data_map_plot(np.max(global_mask, axis=0),
                  name='Min available data mask',
                  filename='min_avail_mask.png')
    data_map_plot(np.min(global_mask, axis=0),
