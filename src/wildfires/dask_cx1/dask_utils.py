...
def get_running_procs(client, user="ahk114", workers=None):
    """Return a string containing `user`'s currently running processes.

    Args:
        client (dask.distributed.Client): Client used to submit functions.
...
def list_notebooks(
    ...
    while (monotonic() - start) < timeout:
        notebooks = client.submit(_list_notebooks, workers=workers, pure=False).result()
        if len(notebooks.split("\n")) >= 1:
            return notebooks
        sleep(0.5)
    else:
...
def setup_remote_jupyter_lab(
    ...
    Returns:
        str: The running notebook(s) including their access tokens.
    ...
    notebooks = list_notebooks(client, jupyter_exc, port, workers)
    return notebooks
