...
def convert_to_netcdf(d):
    ...
    min_day = int(os.path.split((sorted(glob.glob(
        os.path.join(d, '*.dat')))[0]))[1][18:20])
    max_day = int(os.path.split((sorted(glob.glob(
        os.path.join(d, '*.dat')))[-1]))[1][18:20])
    ...
    gs_file = dedent("""
        ...
        'sdfwrite precip'
        """.format(time_str, min_day, max_day, time_str)).strip() + '\n'
    ...
    ctl_file = (dedent("""
        ...
        ENDVARS
        """.format(min_day, calendar.month_abbr[month].lower(), year)).strip()
        + '\n')
    ...
    with open(os.path.join(
        d,
        'GSMaP_NRT_{:}.daily_G.00Z-23Z.ctl'.format(time_str)), 'w') as f:
            f.write(ctl_file)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-d', help=(
        """The directory within which the daily files are stored."""))
