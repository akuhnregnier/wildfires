...
for year, estimate_type, dataset in tqdm(combinations):
    ...
    url = ('ftp://ftp.pbl.nl/hyde/hyde3.2/{:}/zip/{:04d}AD_{:}.zip'
           .format(estimate_type, year, dataset))
    save_as = os.path.join(
            DATA_DIR, 'HYDE', estimate_type,
            '{:04d}AD_{:}.zip'.format(year, dataset))
    if not os.path.isdir(os.path.dirname(save_as)):
        os.makedirs(os.path.dirname(save_as))
    command = "curl --connect-timeout 20 -L -o {:} {:}".format(save_as,
                                                               url)
    os.system(command)
