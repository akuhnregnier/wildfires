...
class IrisStoreBackend(StoreBackendBase, StoreBackendMixin):
    ...
    def load_item(self, path, verbose=1, msg=None):
        ...
        # FIXME:
        print("Iris loading filename:", filename)
    ...
    def dump_item(self, path, item, verbose=1):
        ...
        try:
            ...
            def write_func(to_write, dest_filename):
                # FIXME:
                try:
                    print("In write function!", locals())
                    print("Writing now:")
                    # TODO: Pass options!!
                    iris.save(to_write, dest_filename, saver="nc")

                    # with self._open_item(dest_filename, "wb") as f:
                    #     iris.save(to_write, f.name)
                    #
                    # FIXME
                    print("Wrote file:", os.path.isfile(dest_filename))
                except:
                    import logging

                    print("Write func exception.")
                    from pprint import pprint

                    import sys

                    pprint(sys.exc_info())
                    logging.exception("Dumping exception!")
    ...
    def contains_item(self, path):
        ...
        # FIXME
        print("Contains item:", locals())
        print("Found item:", self._item_exists(filename))

        return self._item_exists(filename)


# class StoreBackendMixin2(object):
...
#
#
# class FileSystemStoreBackend2(StoreBackendBase, StoreBackendMixin2):
#     """A StoreBackend used with local or network file systems."""
#
#     _open_item = staticmethod(open)
#     _item_exists = staticmethod(os.path.exists)
