...
class TestRegrid(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        stacked_start_data = np.vstack(
                [start_data[np.newaxis] for i in range(5)])
        stacked_target_data = np.vstack(
                [target_data[np.newaxis] for i in range(5)])
        ...
        cls.time_source_cube = iris.cube.Cube(
            ...
            dim_coords_and_dims=(
                (iris.coords.DimCoord(
                    ...
                    standard_name='time',
                    units=cf_units.Unit('days since 1970-01-01 00:00:00')), 0),
                (iris.coords.DimCoord(
                    get_centres(np.linspace(90, -90, 5)),
                    standard_name='latitude',
                    units='degrees'), 1),
    ...
    def test_real_regrid(self):
        for method_bool in [True]:
            regridded = regrid(
                    self.source_cube, area_weighted=method_bool,
    ...
    def test_return_identical_cube(self):
        ...
        for method_bool in [True]:
            regridded = regrid(
                    self.target_cube, area_weighted=method_bool,
                    new_latitudes=self.target_cube.coord('latitude'),
                    new_longitudes=self.target_cube.coord('longitude'))
